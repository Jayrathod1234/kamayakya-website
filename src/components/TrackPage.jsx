import {Button, Card, Divider, Loading, Modal, Progress, Text,} from "@nextui-org/react";
...
const TrackPage = () => {
    ...
    const [isSME, setIsSME] = useState(false);  // State to manage SME toggle
    ...
    const filteredRecords = record.filter(
        (item) => (isSME ? item.stock_exchange.includes("SME") : !item.stock_exchange.includes("SME"))
    );
    const handleToggleSME = (sme) => {
        setIsSME(sme);
    };
    ...
    const handleOpenReports = (index) => {
        ...
        if (filteredRecords[index].stock_reports && filteredRecords[index].stock_reports.length > 0) {
            const firstReportUrl = filteredRecords[index].stock_reports[0].document;
    ...
    return (
        ...
        >
            ...
            >
                ...
                <SMEToggle isSME={isSME} toggleSME={handleToggleSME}/>
                ...
                <Grid item container gap={"20px"} justifyContent={"center"}>
                    {filteredRecords.map((item, index) => (
                        <>
                            ...
                            >
                                {filteredRecords[selectedCardIndex] &&
                                filteredRecords[selectedCardIndex].stock_reports &&
                                filteredRecords[selectedCardIndex].stock_reports.length > 0 ? (
                                    <>
                                        <Card
                                            css={{
                                                width: "100%",
                                        ...
                                        >
                                            ...
                                            >
                                                {filteredRecords[selectedCardIndex].stock_name}
                                            </Text>
                                            {filteredRecords[selectedCardIndex].stock_reports.map((report) => (
                                                <IconButton
                                                    key={report.document}
                                                    sx={{
                                                        color: "#000000",
