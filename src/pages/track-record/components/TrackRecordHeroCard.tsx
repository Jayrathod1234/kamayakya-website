...
import IndexedPerformanceChart, { MobileIndexedPerformance } from "./IndexedPerformanceChart";
  ...
  return (
    ...
    <div className={"  rounded-[10px] w-full lg:min-w-[517px]  z-10 relative " + className}>
      ...
      <div className=" flex flex-col md:flex-row gap-3 px-4 pb-4 md:px-0 md:pb-0">
        ...
        <LiveStockPerformanceCard disableExternalLabel={disableExternalLabel} type={type} performance={stockPerformance} newRecommendation={newRecommendation} recommendationLabel={recommendationLabel} />
      </div>
      {/* Middle Section end */}
      {/* Lower Section */}
...
export const TrackRecordHeroCardLanding = ({
  ...
  const recommendationLabel =
    type === "LIVE" ? `${recommendation ?? ""} Live Recommendations` : `${recommendation ?? ""} Exits past`;
  if (!isLoggedIn) {
    return (<div className={" p-4 bg-gray-50 rounded-[10px] w-full   z-10 relative " + className}>
      {!isLoggedIn ? (
        <div className=" w-full max-x-1/2 ">
          <IndexedPerformanceChart renderOnlyMobile={true} />
        </div>
      ) : null}

      {live_recommendations ? live_recommendations: null}
    </div>)
  }

  return (
    // p-4 bg-gray-50
    <div className={"  rounded-[10px] w-full lg:min-w-[517px]  z-10 relative bg-white " + className}>
      {!isLoggedIn ? (
        <div onClick={handleLogin} className="   h-[90%] w-full absolute flex items-center justify-center bottom-0 left-0 z-40">
          <div className="group/lock cursor-pointer shadow-[0px_0px_40px_-9px_rgba(19,135,137,0.46),0px_4px_40px_12px_rgba(118,237,223,0.05)]  overflow-hidden  flex items-center gap-x-[10px] transition-[width] duration-300 h-[56px] w-[56px] hover:w-[234px]   bg-[rgba(255,255,255,1)] rounded-[10px] border border-brand-300">
            <img
      ...
      <div className=" flex px-4 py-4 md:hidden justify-between flex-wrap ">
        <div className=" flex items-center">
          <div className=" p-1 ml-1 shrink-0"></div>
          <p className=" text-md font-bold mr-2 whitespace-nowrap truncate"></p>
          <TrackRecordHeroCardNewChip newRecommendation={newRecommendation} type={type} />
        </div>
      </div>)}
      <div className=" flex flex-col md:flex-row gap-3 px-4 pb-4 md:px-0 md:pb-0">
        <LiveStockPerformanceCard disableExternalLabel={true} type={type} performance={stockPerformance} newRecommendation={newRecommendation} recommendationLabel={recommendationLabel} />
      </div>
    </div>
  );
};

export const TrackRecordHeroCardLandingLocked = ({
  type,
  recommendation,
  averageReturns,
  stockPerformance,
  ...
  return (
    <div className={" p-4 bg-gray-50 rounded-[10px] w-full lg:min-w-[517px]  z-10 relative " + className}>
      ...
      {/* top section end */}
      {/* Middle Section */}
      <div className=" flex flex-col md:flex-row mt-4 gap-3 bg-red-500">
        <AverageReturnCard
          tooltipContent={type === "LIVE" ? <AverageLiveReturnsTooltipContent /> : <AverageExitRetursTooltipContent />}
          type={type}
          averageReturns={averageReturns}
