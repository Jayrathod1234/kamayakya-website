              ...
              <div className="bg-[#fff] bg-[url('/assets/grid.png')] bg-cover rounded-[20px] px-2 py-3 gap-10 text-center">
                <div className=" pt-5   rounded-[10px]">
                  ...
                  <div className=" gap-4  flex ">
                    <StockCardSkeleton
                      length={isMobile ? 1 : 3}
                      className={` ${isMobile ? "w-full mx-auto" :"w-[calc(100%)]" }`}
                    />
                  </div>
                </div>
              </div>
...
export const HotStockSectionVertical = ({ items, isLimitedView, isLoading, error }) => {
  const isMobile = useMediaQuery("(max-width:600px)");
  ...
  return (
    <>
      {isLoading || error ? (
        ...
        <>
          <HotStockSectionSliderVertical items={items} />
        </>
      )}
    </>
  );
