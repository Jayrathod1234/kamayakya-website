...
const FilterMenuTags = () => {
    ...
    const handleButtonClick = (button) => {
        if (clickedButtons.some(clickedButton => clickedButton.id === button.id)) {
            return;
        }
    ...
    const handleCloseClick = (button) => {
        const newClickedButtons = clickedButtons.filter(clickedButton => clickedButton.id !== button.id);
        setClickedButtons(newClickedButtons);
    };
        ...
        <>
            {clickedButtons.map((button) => (
                <div key={button.id} className="w-auto ">
                    <button
                        className={`group relative px-5 py-[10px] flex items-center  justify-between w-full shadow-md border-[#E4E7EC]  border rounded transition-all duration-500 ${clickedButtons.length > 0 ? "bg-brand-500 text-white" : "bg-white text-[#1D2939]"} focus:outline-none`}
                    >
                        <img
                            src={button.icon}
                            alt="Icon"
                            width="18"
                            height="18"
                            className={`group-focus:brightness-0 group-focus:invert ${clickedButtons.includes(button) ? "brightness-0 invert" : ""}`}
                        />
            ...
            {buttons.filter(button => !clickedButtons.some(clickedButton => clickedButton.id === button.id)).map((button) => (
                <div key={button.id} className="w-auto">
                    <button
