      ...
      <div className={cn("text-[#667085] font-semibold truncate flex min-w-0", textCommonClass)}>
        {active ? (
          <span className={cn(` text-4xs ${isBlur || !target_number?.includes('₹') ? "flex items-center": ""}`, targetTextClass)}>
             {targetLabel} {(isBlur || !target_number?.includes('₹')) && activeIcon && <span className=" h-[12px] w-[29px] mx-1 bg-[#FFE8D4] hidden sm:inline-block rounded-full"></span>} | <Active className={` ml-1 ${activeTextClass}`} />
          </span>
        ) : (
          <span className={cn(` text-4xs ${isBlur  || !target_number?.includes('₹')  ? "flex items-center": ""}`, inactiveTextClass)}>{targetLabel ? targetLabel : ""} {(isBlur  || !target_number?.includes('₹')) && activeIcon && <span className=" h-[12px] w-[29px] mx-1 bg-[#8e95a3] hidden sm:inline-block rounded-full"></span>} {targetLabel && "|"} <span className=" ml-1 font-medium">Inactive</span></span>
        )}
      </div>
    </ChipContainer>
  );
