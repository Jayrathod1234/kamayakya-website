    ...
    <div className="max-w-full pe-2">
      ...
      <div className="flex justify-between text-gray-600 mt-2">
        {prices.map((price, index) => (
          ...
          >
            <div className={`block text-[#EDF0F5] font-semibold text-sm w-14 bg-[#EDF0F5] rounded-full my-2`}>
              ₹{price}
            </div>
            {index === labels.length - 1 ? (
              <span className="text-orange-500 block blur-none font-normal">
                Active
              </span>
            ) : (
              <span className="block text-[#F9FAFB] font-semibold text-sm w-20 bg-[#F9FAFB] rounded-full">
                {dates[index]}
              </span>
            )}
          </div>
