...
export function  StatsCard() {
  return (
    <div className="mx-auto max-w-[90%] md:max-w-[600px] open_sans rounded-2xl bg-white shadow-md flex  justify-between items-center px-2 py-3 xl:px-6 xl:py-[25px] space-y-0 md:space-y-0 md:space-x-6 -mt-16 z-20 relative">
      {stats.map((stat, index) => (
        ...
        >
          ...
          <span className="text-[13px] lg:text-[16px] xl:text-[18px] text-gray-950 font-semibold">{stat.label}</span>
        </div>
      ))}
    </div>
  );
    ...
    // max-lg:hidden py-[71px]
     className='  relative overflow-hidden bg-[url("/landing/stat_grid.png")] bg-cover'>
      <div className=' h-full '>
        <div className=''>
          <div >
            <video poster="/landing/stat_poster.png" autoPlay loop muted className='h-full w-full max-h-[665px] max-w-[975px] mx-auto shadow-md rounded-[20px] lg:rounded-[70px] ' height={665} width={975} src="/hero_company/KMK-V1 (1).mp4"></video>
          </div>
          <div className=''>
            <StatsCard/>
          </div>
