  ...
  return (
    ...
    >
      <div className=" flex-shrink-0">
        <img
          className="rounded-full object-cover max-h-7 max-w-7 w-full h-fullz"
          height={28}
          width={28}
          src={logo}
          alt="company-logo"
...
interface TimelineProps {
  targets: any[];
  onReportClick: (target: any) => void;
}

function Timeline({ targets, onReportClick }: TimelineProps) {
          ...
          <CarouselItem key={index} className={`-pl-0 basis-auto`}>
            <div className="w-[240px]">
              <div className=" text-center flex flex-col items-center justify-center">
                <p className=" pb-3 text-2xs text-gray-600">
                  {format(new Date(target.report_date ?? target.youtube_date), "dd MMM yy")}
                </p>
                ...
                <div className=" px-3 py-5 mt-3 w-full bg-white max-w-[224px] rounded-2xl border">
                  <p className=" text-2xs font-semibold truncate max-w-full">
                    {target.report_action_text ?? "Video Released"}
                  </p>
                  <p className=" text-sm font-bold truncate max-w-full">{target.report_name ?? target.youtube_title}</p>
                  <button
                    onClick={() => onReportClick(target)}
                    className=" mt-3  w-full py-[7px] pl-3 pr-[14px] border border-gray-200 rounded-[4px] flex items-center justify-center gap-x-1"
                  >
                    {target.type === "report" ? <FileText size={16} /> : <Video size={16} />}
                    <p className=" text-2xs text-gray-950 font-medium max-w-[88px] truncate">
                      {target.report_name ? "Page Report" : "Show Video"}
                    </p>
                  </button>
                </div>
              </div>
...
export default function SampleReport() {
  ...
  function selectCompany(companyData: any) {
    setCompanySelected(companyData);
  }

  const handleReportClick = async (target: any) => {
    const mp = getMixPanelClient();
    const usertype = await getUserType();
    mp.track("samplereports_clicked", {
      page: "Homepage",
      usertype: usertype,
    });
    window.open(target.document ?? target.youtube_link, "_blank");
  };
      ...
      <div className=" py-[50px] sm:py-[100px] open_sans rounded-[28px]">
        ...
        <div className=" pt-[46px] sm:pt-0">
          ...
          <div className="  mt-5 border border-gray-100 bg-gray-50 rounded-xl p-9">
            ...
            <div className="">
              <Timeline targets={companySelected?.timeline || []} onReportClick={handleReportClick} />
            </div>
          </div>
        </div>
      </div>
