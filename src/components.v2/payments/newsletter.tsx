...
export function Newsletter({ page = "Pricing_Page" }) {
  ...
  const handleNewsLetterEmailSubmit = async () => {
    try {
      ...
      if (e?.response?.data?.email[0]?.includes("exists")) {
        toast({
          variant: "warn",
          title: "Email Already Subscribed!",
          description:
            "Looks like you're already on the list! Sit back and enjoy the investment wisdom coming your way.",
        });
        return;
      }
      if (e?.message.includes("Network Error")) {
        toast({
          variant: "danger",
          title: "Network Issue!",
          description: "Network issue detected! Please check your connection and try subscribing again.",
        });
        return;
      }
      toast({
        variant: "danger",
        title: "Error",
        description: "Something went wrong on our end.Give it another shot, or contact us if the issue persists.",
      });
    } finally {
      setEmail("");
      setLoading(false);
