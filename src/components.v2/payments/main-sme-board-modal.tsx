...
const BoardHead = ({head}: { head: string }) => {
    return <h2
        className="hidden md:block text-xl font-bold text-gray-950 m-0 sticky top-[0px] pt-[16px] bg-inherit ">{head}</h2>;
};

const Tag = ({tag}: { tag: string }) => {
    return (
        <span
            className="uppercase px-3 py-[6px] pl-[10px] border border-[#E4E7EC80] rounded-[20px] text-2xs text-[#344054]">
      {tag}
    </span>
    );
};
...
export function MainSmeBoardModal({trigger}: { trigger: React.ReactNode }) {
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent
                className=" contact__modal !rounded-[20px] p-4 md:max-w-[680px] lg:max-w-[1000px] h-[80dvh] max-h-[477px] md:max-h-[858px] lg:max-h-[718px] mt-10 overflow-hidden md:overflow-y-scroll pricing">
